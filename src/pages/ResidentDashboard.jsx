  ...
  return (
    <div className="p-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        ...
        {(!user?.roomNumber || user.roomNumber === 'N/A' || user.roomNumber === '') ? (
          <div 
            onClick={() => alert("You need to be assigned to a room before submitting maintenance requests.")}
            className="bg-gray-400 text-white p-4 rounded text-center cursor-not-allowed opacity-80 shadow-sm"
          >
            <div className="text-2xl mb-2">🔧</div>
            Maintenance (Room Required)
          </div>
        ) : (
          <Link to="/maintenance" className="bg-yellow-600 text-white p-4 rounded text-center hover:bg-yellow-700 transition-colors shadow-sm">
            <div className="text-2xl mb-2">🔧</div>
            Maintenance
          </Link>
        )}
